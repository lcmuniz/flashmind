...
export default function DeckView({ deck, cards, userEmail, onBack, onStudy, onAddCard, onDeleteCard }) {
    ...
    useEffect(() => {
        const handleKeyDown = (e) => {
            ...
            if (e.key.toLowerCase() === 'e') {
                if ((stats.new > 0 || stats.due > 0) && !showAdd) onStudy();
            }
        };
        window.addEventListener('keydown', handleKeyDown);
        return () => window.removeEventListener('keydown', handleKeyDown);
    }, [onStudy, stats, showAdd]);
            ...
            <div className="bg-white border border-slate-200 rounded-[32px] p-8 mb-8 shadow-sm">
                <div className="flex flex-col sm:flex-row justify-between items-center gap-6">
                    ...
                    <div className="flex gap-2 w-full sm:w-auto">
                        <button onClick={() => setShowAdd(true)} className="flex-1 sm:flex-none py-3 px-6 rounded-2xl border-2 border-slate-100 text-sm font-bold">Add Cartão</button>
            ...
            <div className="space-y-3">
                {cards.map(card => (
                    <div key={card.id} className="bg-white border border-slate-100 p-4 rounded-2xl flex justify-between items-center text-sm hover:border-indigo-200 transition-all group">
                        ...
                        <button onClick={() => onDeleteCard(card.id)} className="text-slate-300 hover:text-red-500 p-2 transition-colors opacity-0 group-hover:opacity-100"><Trash2 size={18} /></button>
                    </div>
                ))}
            </div>
            ...
            {showAdd && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-[32px] p-8 max-w-lg w-full shadow-2xl scale-in">
                        ...
                        <textarea className="w-full border-2 border-slate-100 bg-slate-50 rounded-2xl p-4 min-h-[100px] text-sm mb-4 outline-none focus:border-indigo-500 focus:bg-white transition-all font-medium" value={front} onChange={(e) => setFront(e.target.value)} placeholder="Frente do cartão (pergunta)" />
